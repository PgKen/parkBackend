...
router.get("/listcustomerforidbuildandzonepayforadmin", (req, res) => {
  ...
  function mapData() {
    ...
    let mData = data.map((item) => {
      ...
      if (moment(item.nowdate).format("YYYY-MM-DD") == today) {
        pricepay = item.pricepay;
        backdebit = totalpay - item.price + item.nowpaid + item.debit;
        // backdebit = totalpay + item.nowpaid + item.debit;
      } else {
        pricepay = 0;
        backdebit = totalpay - item.price + item.debit;
        // backdebit = totalpay + item.debit;
      }
...
router.post("/paymoney", (req, res) => {
  ...
  let oldpay = 0;
  let g_idUser = 0;
  let gDebit = 0; // เงินค้างที่ต้องจ่าย
  ...
  function getDebitPalOld(){
    // รับเงินเกินยอดเก่า
    let id = req.body.idboxs;
    let money = req.body.selectpay;

    let sql = "SELECT debit FROM boxs ";
    sql += "WHERE id = ?";
    conn.query(sql, [id], (err, resp) => {
      if (err) {
        console.log("[mysql err]", err);
      ...
      gDebit = resp[0].debit; // รับเงินค้างยอดเก่า
      updateDebit();
    });
  }
  ...
  function updateDebit() {
    let id = req.body.idboxs;
    let selectpay = req.body.selectpay;
    selectpay =  parseFloat(gDebit) - parseFloat(selectpay);
    if(selectpay < 0){
      selectpay = 0;
    }
    let sql = "UPDATE boxs SET debit = ? ";
    sql += "WHERE id = ?";
    conn.query(sql, [selectpay, id], (err, resp) => {
      if (err) {
        console.log("[mysql err]", err);
      }
      getUnitDate();
  ...
  function getUnitDate() {
    console.log("gDebit = " + gDebit);
    // รวมยอดเกินเก่า + ยอดจ่ายใหม่ / รายวัน  = หาวันที่ลงตัว
    // จำนวนวันที่หารลงตัว
    let selectpay = req.body.selectpay; // จำนวนเงินกรอก
    let id = req.body.idboxs;
    let totaldate = (parseFloat(selectpay) + parseFloat(gOverPayOld) - parseFloat(gDebit)) / gMoney; // เงินกรอก + เงินเกิน / รายวัน
    gTotalPay = parseFloat(selectpay) + parseFloat(gOverPayOld) - parseFloat(gDebit);
...
router.post("/paybill", (req, res) => {
  ...
  let oldpay = 0;
  let g_idUser = 0;
  let gMoneyDebit = 0;  // ยอดเงินค้างชำระ - เงินที่จ่าย
  ...
  function getPay(){
    ...
    conn.query(sql, [id], (err, resp) => {
      ...
      gMoneyDebit = resp[0].debit - selectpay; // รับเงินค้างชำระ
      recordDebit();
    });
  ...
  function getDebit(){
    ...
    let id = req.body.idboxs;
    // let selectpay = req.body.selectpay; // จำนวนเงินกรอก
    let selectpay = gMoneyDebit; // จำนวนเงินกรอก
  ...
  function recordDebit(){
    // บันทึกยอดเงินค้างชำระ
    let id = req.body.idboxs;

    let sql = "UPDATE boxs SET debit = ? WHERE id = ? ";
    conn.query(sql, [gMoneyDebit, id], (err, resp) => {
      if (err) {
        console.log("[mysql err]", err);
      }
      getUnitDate();
  ...
  function getUnitDate() {
    ...
    let totaldate = (parseFloat(gMoneyDebit) + parseFloat(gOverPayOld)) / gMoney; // เงินกรอก + เงินเกิน / รายวัน
    //gTotalPay = parseFloat(selectpay) + parseFloat(gOverPayOld);
    gTotalPay = parseFloat(gMoneyDebit) + parseFloat(gOverPayOld);
...
router.get("/daytotal", (req, res) => {
  ...
  function getData() {
    ...
    let sql = "SELECT payrecord.id,payrecord.datepay,"
    sql += "staff.`name`,staff.lastname,boxs.idbuild,"
    sql += "build.buildname,payrecord.pricepay,"
    sql += "sum(pricepay) As totalpay,"
    sql += "payrecord.idstaff,payrecord.idboxs,boxs.idzone,"
    sql += "zone.zonename "
    sql += "FROM payrecord "
    sql += "LEFT JOIN staff ON payrecord.idstaff = staff.id "
    sql += "LEFT JOIN boxs ON payrecord.idboxs = boxs.id "
    sql += "LEFT JOIN build ON boxs.idbuild = build.id "
    sql += "LEFT JOIN zone ON boxs.idzone = zone.id "
    sql += "WHERE payrecord.idstaff = ? AND payrecord.datepay = ? "
    sql += "GROUP BY idbuild,idzone";

    conn.query(sql, [iduser, date], (err, resp) => {
      if (err) {
        console.log("[mysql err]", err);
      }
      ...
      setTimeout(() => {
        if (dataprice.length > 0) {
          nameStaff = resp[0].name + " " + resp[0].lastname;
          fnTotalPrice();
        } else {
          sendData();
        }
    ...
    for (let i = 0; i < datarole.length; i++) {
      conn.query(
        ...
        (err, resp) => {
          ...
          datarole[i].sumpaid = resp[0].sumpaid;
...
router.post('/bill', (req, res) => {
  ...
  function getData() {
    ...
    let valSql = [];
    valSql.push(startDate);
    valSql.push(endDate);


    let sql = "SELECT bill.idbill,bill.book,bill.bnumber,"
    sql += "bill.bdate,bill.bmoney,bill.bstaff,bill.bidboxs,"
    sql += "boxs.id,boxs.idcustomer,customer.id,customer.`name` As customername,"
    sql += "customer.lastname As customerlastname,staff.`name`,"
    sql += "staff.lastname,staff.id As idStaff ";
    sql += "FROM bill ";
    sql += "INNER JOIN boxs ON bill.bidboxs = boxs.id ";
    sql += "INNER JOIN customer ON boxs.idcustomer = customer.id ";
    sql += "INNER JOIN staff ON bill.bstaff = staff.id ";
    sql += "WHERE bdate BETWEEN ? AND ? ";
  ...
  function mapBill() {
    let mDataBill = dataBill.map((item) => {
      let mDate = moment(item.bdate).format("DD-MM-YYYY");
      let mName = item.customername + " " + item.customerlastname;
      item.bdate = mDate;
      item.name = mName;
      return item;
    });
  ...
  function sumBill() {
    ...
    let sql = "SELECT Sum(bmoney) As summoney ";
    sql += "FROM bill ";
    sql += "WHERE bdate BETWEEN ? AND ? ";
...
router.post("/resetdata", (req, res) => {
  ...
  function getData() {
    ...
    if (idbuild == "9999" && idzone == "9999") {
      let sql = "UPDATE boxs SET curdate = ? ,nowdate = ? , startdate = ?,";
      sql += " recogdate = ?,nowpaid = 0 ,overpay = 0 ,debit = 0 ";
      sql += "WHERE 1 ";

      conn.query(sql, [date, date, date, date], (err, resp) => {
        if (err) {
          console.log("[mysql err]", err);
        }
        // selectidboxs();
      ...
      conn.query(sql, [date, date, date, date, idbuild, idzone], (err, resp) => {
        if (err) {
          console.log("[mysql err]", err);
        }
        // selectidboxs();
